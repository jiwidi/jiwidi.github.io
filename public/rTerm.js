...
window.rTerm = function (options) {
	...
	this.onInternalLink = typeof options.onInternalLink === 'function'
		? options.onInternalLink
		: null;
	// Starting path in fs
	...
	this.buildPrompt = function (cdir) {
		var displayed = (cdir || '').replace('/home/jiwidi/', '~/').replace('/home/jiwidi', '~');
		if (displayed && displayed.slice(-1) !== '/') displayed += '/';
		return '<b>' + this.uhsername + '</b>' +
			'<span class="prompt-sep">:</span>' +
			'<span class="prompt-path">' + displayed + '</span>' +
			'<span class="prompt-sigil">$</span>&nbsp;&nbsp;';
	};
	this.termPrev = this.buildPrompt(this.cdir);
	this.oldInput = ''
	this.input = '';
	this.nStrings = 0;
	this.commandHistory = [];
	...
	this.callUpstart = function () {
		...
		this.callUpstartImmediately(this.data.upstart);
		return 0;
	};
	...
	this.callPhotos = function () {
		this.upcid = 0;
		this.upstartInterrupted = false;
		this.callUpstartImmediately(this.data.startphotos || []);
		return 0;
	};
	...
	this.callKeyboard = function () {
		this.upcid = 0;
		this.upstartInterrupted = false;
		this.callUpstartImmediately(this.data.startkeyboards || []);
		return 0;
	};
	...
	this.callBlog = function () {
		this.upcid = 0;
		this.upstartInterrupted = false;
		this.callUpstartImmediately(this.data.startblog || []);
		return 0;
	};
		...
		for (lid in command) {
			setTimeout(function () {
				if (window.blurred && !this.interruptCommand) {
					this.enterCommandImmediately(command.slice(
						this.currlid));
	...
	// Update #termcli with new input. Caps oldInput to maxStrings lines
	// without forced reflows (no offsetHeight reads).
	this.updateTerm = function () {
		if (typeof this.maxStrings === 'number' && this.maxStrings > 0 && this.nStrings > this.maxStrings) {
			var lines = this.oldInput.split('<br>');
			var keep = this.maxStrings;
			if (lines.length > keep) {
				lines = lines.slice(lines.length - keep);
				this.oldInput = lines.join('<br>');
				this.nStrings = keep;
			}
		}
		var el = document.getElementById("termcli");
		if (el) el.innerHTML = this.oldInput + this.termPrev + this.input;
	};

	// Kept for backwards compat; no-op fallback.
	this.delFristString = function () {
		var cutPos = this.oldInput.indexOf("<br>");
		if (cutPos >= 0) this.oldInput = this.oldInput.slice(cutPos + 4);
		this.nStrings = Math.max(0, this.nStrings - 1);
	};
	...
	this.enterCallback = (function () {
		if (this.input == '') {
			...
			if (this.commandHistory.length > this
				.maxHistoryLength) {
	...
	this.upCallback = (function () {
		if (this.commandHistoryIterator < this.commandHistory
			.length - 1) {
			this.commandHistoryIterator++;
		}
		this.input = this.commandHistory[this
	...
	this.sendString = (function (cli_input) {
		...
		request.open(
			"GET",
			"https://" + window.location.hostname + ":" + this
			.loggerAppPort + "?" + cli_input,
			true);
		request.send();
	}).bind(this);
				...
				if (!item.startsWith('.') || all) {
					if (typeof dirData[item] === 'string') {
						if (dirData[item].startsWith("_link:")) {
							this.oldInput +=
								'<a class="link" href="' + dirData[
									item].slice(6, ) +
								'" target="_blank">' + item +
								'</a><br>';
						} else if (dirData[item].startsWith(
								"_ilink:")) {
							this.oldInput +=
								'<a class="link" href="' + dirData[
									item].slice(7, ) +
						...
						this.oldInput += '<font color="#729FCF">' +
							item + '</font><br>';
					}
					this.nStrings++;
				}
	...
	this.catCallback = (function (args) {
		...
		if (data == '' || typeof data === 'undefined') {
			...
			if (data.startsWith("_call:")) {
				...
				var ipath = data.slice(7);
				this.oldInput += this.termPrev + this.input +
					'<br>opening <a class="link" href="' + ipath +
					'" target="_self">' + ipath + '</a> in preview…<br>';
				if (typeof this.onInternalLink === 'function') {
					this.onInternalLink(ipath);
				}
			} else if (data.startsWith("_link:")) {
				var elink = data.slice(6);
				this.oldInput += this.termPrev + this.input +
					'<br>opening <a class="link" href="' + elink +
					'" target="_blank">' + elink + '</a> in a new tab…<br>';
		...
		if (typeof data === 'undefined') {
			...
			if (data.startsWith("_link:")) {
				...
				if (typeof this.onInternalLink === 'function') {
					...
					window.open(ipath, '_self').focus();
				}
			} else {
				this.oldInput += this.termPrev + this.input +
					'<br>' + this.input + ": Not a directory" +
					'<br>';
				this.input = '';
				this.nStrings += 2;
			}
	...
	this.aptCallback = (function (args) {
		if (args.length == 1) {
			...
			this.oldInput += this.termPrev + this.input +
				...
				'<br>E: Unable to locate package ' + args[2] +
				'<br>';
			this.nStrings += 5;
		} else {
			this.oldInput += this.termPrev + this.input +
